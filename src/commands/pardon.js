...
async function pardonHandler(interaction) {
	...
	for (const userId of userIds) { 
        ...
        if (type === 'warn_type') { // If chosen warn within options
            let { userWarns } = await Warnings.findAndCountAll({ // Grab all warns
                where: {
                    user_id: member.id
                }
            });
            ...
            let { userKicks } = await Kicks.findAndCountAll({ // Grab all kicks
                where: {
                    user_id: member.id
                }
            });
